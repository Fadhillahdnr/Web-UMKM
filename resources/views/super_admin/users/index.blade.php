...
<head>
    ...
    <title>Kelola User - Super Admin UMKM Makanan</title>
    <style>
...
<body>
    ...
    <div class="container">
        ...
        <a href="{{ route('super_admin.users.create') }}" class="btn btn-success" style="margin-bottom: 20px;">+ Tambah User</a>
                        ...
                        <tr>
                            ...
                            <td>
                                <span style="padding: 3px 8px; border-radius: 4px; background: 
                                    {{ $user->role->name == 'super_admin' ? '#AD1457' : ($user->role->name == 'admin' ? '#E91E63' : '#FFB74D') }}; 
                                    color: white;">
                                    {{ ucfirst($user->role->name) }}
                                </span>
                            </td>
                            <td>
                                <a href="{{ route('super_admin.users.edit', $user->id) }}" class="btn btn-primary">Edit</a>
                                <form method="POST" action="{{ route('super_admin.users.destroy', $user->id) }}" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus?');">
                                    @csrf
                                    @method('DELETE')
